...
- **Do not commit credentials.** The only secret is `COMET_PASSWORD`, fetched from Doppler CLI via `doppler.yaml` (never process-env injection). Host (`192.168.0.126`) and username (`admin`) are non-sensitive and safe in code/config. See `docs/reference/comet-api.md#security-model`.
...
- **Credentials:** The only secret is `COMET_PASSWORD`, fetched from Doppler CLI via `doppler.yaml` (never process-env injection). Host (`192.168.0.126`) and username (`admin`) are non-sensitive and safe in code/config. See `docs/reference/comet-api.md#security-model`.
