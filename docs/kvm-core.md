...
Connections are per-session. `kvm_connect(host, password?, username="admin")` accepts an explicit password or fetches `COMET_PASSWORD` from the Doppler CLI using `doppler.yaml` (`secrets_managment`/`dev`). The process environment is not used for the Comet password. The bundled launcher is plain `uv run --locked --python 3.13 python ./glkvm_mcp.py`; Doppler must be installed and authenticated on the host.
...
- LAN-first operation.
- Per-session password supplied through `kvm_connect`, or fetched from Doppler CLI (`COMET_PASSWORD` secret).
