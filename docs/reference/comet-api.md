...
- **Browser automation caveat** — do not drive the Comet web UI with a browser automation driver. The self-signed certificate lands Chrome on its certificate interstitial, and that interstitial is not automation-attachable (`Cannot attach to this target`; `screenshot`/`read_page` fail with `Frame … showing error page`). The `thisisunsafe` bypass needs a focused, typeable page, which cannot exist when attach fails. Agents must use `curl -k` or this MCP server over the HTTP/WebSocket API, never a browser driver. If a browser is genuinely required, pre-trust the Comet certificate in the profile first and cross-check its fingerprint.
