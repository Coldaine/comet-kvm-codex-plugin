...
> **Status:** Draft from prior design session (2026-07-07). Needs reconciliation with `docs/decisions.md` (D3, D4, D7, D8) and `docs/NORTH_STAR.md` cartography spike scope. Key areas to update: storage location (should be on-Comet or host data dir, not `/ledger`), state engine integration, and VLM prompting pattern.
...
The Cartographer compiles its findings into a **Semantic Capability Index** JSON file saved to the `/ledger`, stripping visual noise and presenting only actionable controls.
